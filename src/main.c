 ...
 */
static void free_command_chain(Command *head) {
  // We skip the head node because it is allocated on the stack in main
  if (head == NULL)
    return;
 ...
 */
static void restore_std_fds(Command *cmd, int saved_stdin) {
