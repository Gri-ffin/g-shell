...
int main() {
    ...
    while (true) {
        ...
        // If i don't do this, since parse commands distinguishes commands
        // only `ls` would register for `ls && cat foo` etc...
        if (*input != '\0') {
            HIST_ENTRY *last_entry = history_length > 0 ? history_get(history_length) : NULL;

            // Dedup
            if (!last_entry || strcmp(input, last_entry->line) != 0) {
                add_history(input);
            }
        }
